...
async function main() {
  ...
  const VaultErrorController = await ethers.getContractFactory(
  ...
  if (deploymentInfo["VaultErrorController"].address == "") {
    ...
    deploymentInfo["VaultErrorController"].address =
      vaultErrorController.address;
  } else {
    vaultErrorController = VaultErrorController.attach(
      deploymentInfo["VaultErrorController"].address
    );
  }
  ...
  if (deploymentInfo["ShortsTracker"].address == "") {
    ...
    shortsTracker = ShortsTracker.attach(
      deploymentInfo["ShortsTracker"].address
    );
  }
  ...
  if (deploymentInfo["Router"].address == "") {
    ...
    deploymentInfo["Router"].constructorArgs = [
      vault.address,
      usdg.address,
      weth.address,
    ];
  ...
  if (deploymentInfo["GlpManager"].address == "") {
    glpManager = await GLPManager.deploy(
      vault.address,
      usdg.address,
      glp.address,
    ...
    deploymentInfo["GlpManager"].constructorArgs = [
      vault.address,
      usdg.address,
      glp.address,
      shortsTracker.address,
  ...
  if (deploymentInfo["PositionUtils"].address == "") {
    ...
    positionUtils = PositionUtils.attach(
      deploymentInfo["PositionUtils"].address
    );
  }
  ...
  if (deploymentInfo["PositionRouter"].address == "") {
    ...
    deploymentInfo["PositionRouter"].constructorArgs = [
      vault.address,
      router.address,
      weth.address,
      shortsTracker.address,
    ...
    positionRouter = PositionRouter.attach(
      deploymentInfo["PositionRouter"].address
    );
  }
  ...
  if (deploymentInfo["Vault"].configure == true) {
    ...
    // Set fees
    if (deploymentInfo["Vault"].configure_args.configureFees) {
      console.log("\tSetting fees...");
      await vault.setFees(
        deploymentInfo["Vault"].configure_args.fees.taxBasisPoints,
        deploymentInfo["Vault"].configure_args.fees.stableTaxBasisPoints,
        deploymentInfo["Vault"].configure_args.fees.mintBurnFeeBasisPoints,
        deploymentInfo["Vault"].configure_args.fees.swapFeeBasisPoints,
        deploymentInfo["Vault"].configure_args.fees.stableSwapFeeBasisPoints,
        deploymentInfo["Vault"].configure_args.fees.marginFeeBasisPoints,
        deploymentInfo["Vault"].configure_args.fees.liquidationFeeUsd,
        deploymentInfo["Vault"].configure_args.fees.minProfitTime,
        deploymentInfo["Vault"].configure_args.fees.hasDynamicFees
      );
    }

    // Configure tokens
    if (deploymentInfo["Vault"].configure_args.configureTokens) {
      for (const key in deploymentInfo["Vault"].configure_args.token_configs) {
        const config =
          deploymentInfo["Vault"].configure_args.token_configs[key];
        console.log(`\tConfiguring ${key} (${config.tokenAddress})...`);
  ...
  // Configure errors
  if (deploymentInfo["VaultErrorController"].configure == true) {
    console.log("Configuring errors...");
  ...
  for (const contractName in deploymentInfo) {
    if (
      deploymentInfo[contractName].address != "" &&
      deploymentInfo[contractName].verify == true
    ) {
